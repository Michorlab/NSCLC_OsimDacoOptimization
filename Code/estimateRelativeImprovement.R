...
files = list.files("../Results_102719/Corr0_noPreRes/")
for (file in files){
  load(paste0("../Results_102719/Corr0_noPreRes/", file))
}
rm(file, files)
...
# Compare all 3 levels of drug schedules
comps = tibble(lev1 = compSched(group1.1Count, group2.1Count),
               lev2 = compSched(group1.2Count, group2.2Count),
               lev3 = compSched(group1.3Count, group2.3Count))
...
comps %>%
  ...
  # Add ID variable
  mutate(id = 1:100) %>%
  # Select every 10th observation since plot looks crowded
  # filter(id %% 10 == 0) %>%
  gather(key = sched, value = improvement, -id) %>%
  # mutate(sched = recode(sched, lev1 = "Level 1", lev2 = "Level 2", lev3 = "Level 3")) %>%
...
# ggsave("../Figures//heavierPatients_1yearWaterfallPlot.pdf", width = 5, height = 5)
