...
# Description: Obtain growth rates in HCC827 cell line, expected to be resistant to daco
#####
...
#### Day 1 of PC9-DRH
# Load data
ccOsi = read.csv("Data from Scott/HCC827_Osi/Day 1-HCC827_Osi.csv", header = F,
                 stringsAsFactors = F)
ccDaco = read.csv("Data from Scott/HCC827_Daco/Day 1-HCC827_Daco.csv", header = F,
                  stringsAsFactors = F)
...
#### Day 2 of PC9-DRH
ccOsi = read.csv("Data from Scott/HCC827_Osi/Day 2-HCC827_Osi.csv", header = F,
                 stringsAsFactors = F)
ccDaco = read.csv("Data from Scott/HCC827_Daco/Day 2-HCC827_Daco.csv", header = F,
                  stringsAsFactors = F)
# Modify data for easier analysis
ccOsi[1,dim(ccOsi)[2]] = 0
ccOsi$V1 = as.numeric(ccOsi$V1)
...
#### Day 3 of PC9-DRH
ccOsi = read.csv("Data from Scott/HCC827_Osi/Day 3-HCC827_Osi.csv", header = F,
                 stringsAsFactors = F)
ccDaco = read.csv("Data from Scott/HCC827_Daco/Day 3-HCC827_Daco.csv", header = F,
                  stringsAsFactors = F)
# Modify data for easier analysis
ccOsi[1,dim(ccOsi)[2]] = 0
ccOsi$V1 = as.numeric(ccOsi$V1)
...
ccFrame %>%
  ...
  scale_y_continuous(name = "Cell Count for HCC827", labels = scales::math_format(expr = 10^.x))+
  labs(x = "Time (hr)", color = "Osimertinib\nlevel")+
  theme_bw(base_size = 20)
# ggsave("Figures/cellCount_HCC827osi.pdf", width = 7, height = 5)
...
ccFrame %>%
  ...
  theme_bw(base_size = 20)
# ggsave("Figures/cellCount_HCC827daco.pdf", width = 7, height = 5)

# # Make figures to observe trend
# # pdf("../Figures/cellCount_PC9DRH.pdf", width = 10, height = 4, pointsize = 7)
# g1 = ggplot(ccFrame[ccFrame$daco==0,], aes(x = hr, y = log(cellCount),
#                                            color = as.factor(osi)))+
#   geom_point()+geom_smooth(method = "lm")+
#   labs(x = "Time (hr)", color = "Osimertinib\nlevel")+theme_bw()
# g2 = ggplot(ccFrame[ccFrame$osi==0,], aes(x = hr, y = log(cellCount),
#                                           color = as.factor(daco)))+
#   geom_point()+geom_smooth(method = "lm", level = 0)+
#   labs(x = "Time (hr)", color = "Dacomitinib\nlevel")+theme_bw()
# grid.arrange(g1,g2,nrow = 1)
# # dev.off()

HCC827growth = ccFrame %>%
...
# Specify death rate
if (min(HCC827growth$growth)<0){
  d0_HCC = abs(min(HCC827growth$growth))
}else{
  d0_HCC = 0.001
}

# Fit is a bit strange for daco
ymin_HCC = min(HCC827growth$growth)-10^-5
ymax_HCC = max(HCC827growth$growth)+10^-5
HCC827fit = lm(log((growth-ymin_HCC)/(ymax_HCC-growth))~ I(log(log(daco+1)+1))+
                 I(log(osi+1)^(1/2)), HCC827growth)

convertGrowth = function(growth, trans = F){
  if (trans){log((growth- ymin_HCC)/(ymax_HCC - growth))}
  else{(ymax_HCC*exp(growth)+ymin_HCC)/(1+exp(growth))}
}

HCC827growth %>% 
  mutate(fit = predict(HCC827fit, newdata = .)) %>% 
  mutate(fit = (ymax_HCC*exp(fit)+ymin_HCC)/(1+exp(fit))) %>% 
