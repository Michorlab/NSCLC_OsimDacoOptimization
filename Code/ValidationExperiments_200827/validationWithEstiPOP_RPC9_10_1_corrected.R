...
#                    osi = c(0, 0, 10.38, 5.19, 5.19, 2*10.38, 2*10.38, 1.5*20.75))
drugCombo = tibble(daco = c(0, 4.40, 0, 1.47, 2.94, 1.47, 4.40, 2.94),
...
bT790M = birth_t790m(daco = drugCombo$daco[6], osi = drugCombo$osi[6]) - d_t790m +0.01
...
bT790M = birth_t790m(daco = drugCombo$daco[8], osi = drugCombo$osi[8]) - d_t790m + 0.01
...
### Load observations
obs_pool = read_csv("../../Validation data/012420/pc9_rpc9_10_1.csv")
...
pc9_10to1pool_comparison = full_join(obs_pool, comboPredPool, by = c("condition", "day"))
# write_csv(pc9_10to1pool_comparison, "../../Validation data/Simulations_200827/rpc9_10to1.csv")
# Plot data: predicted is shown in lines, observed are dots with error bars
pc9_10to1pool_comparison %>% 
  ungroup() %>% 
  ...
  geom_errorbar(aes(ymin = medianCount - sdCount/2, ymax = medianCount + sdCount/2), width = 2, show.legend = F) +
  ...
  labs(x = "Day on Treatment", color = "Condition", title = "PC9-RPC9 10:1 Pool") +
  facet_wrap(~condition) +
  theme_bw()
# ggsave("../../Figures/validation_PfizerResults_PC9toRPC9_10to1pool_200827.pdf", width = 8, height = 6)
